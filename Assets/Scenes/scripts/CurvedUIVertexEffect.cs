...
namespace CurvedUI
{
    ...
    public partial class CurvedUIVertexEffect : BaseMeshEffect
    {
        ...
        public static List<float> m_curvedVertsAngles = new List<float>();
        public static List<float> m_transparentDepth = new List<float>();
        UIVertex m_ret;
        ...
        protected override void Start()
        {
            ...
            if (myCanvas && GetComponent<Selectable>()) //Not Necessary
            {
        ...
        UIVertex GetMaximumXVertex(List<UIVertex> vertices)
        {
            if (vertices == null || vertices.Count == 0)
            {
                Debug.LogError("No vertices in the list.");
                return default(UIVertex);
            }

            UIVertex minXVert = vertices[0]; // Initialize for the leftmost vertex
            UIVertex maxXVert = vertices[0]; // Initialize for the rightmost vertex

            // Identify the extreme leftmost and rightmost vertices
            foreach (UIVertex vert in vertices)
            {
                if (vert.position.x < minXVert.position.x)
                {
                    minXVert = vert;
                }
                if (vert.position.x > maxXVert.position.x)
                {
                    maxXVert = vert;
                }
            }

            // Determine which of these two vertices is further along the z-axis
            UIVertex farthestZVert = (Mathf.Abs(minXVert.position.z) > Mathf.Abs(maxXVert.position.z)) ? minXVert : maxXVert;

            return farthestZVert;
        }
        ...
        public override void ModifyMesh(VertexHelper vh)
        {
            ...
            if (tesselationRequired || !Application.isPlaying)
            {
                ...
                vh.GetUIVertexStream(m_tesselatedVerts);//6 vertices

                //subdivide them
                TesselateGeometry(m_tesselatedVerts);//28 vertices
            ...
            if (curvingRequired)
            {
                ...
                string strObjectName = this.transform.gameObject.name;

                if (strObjectName == "CanvasWebViewPrefabView")
                {
                    m_curvedVertsAngles.Clear();
                }

            
                if (strObjectName == "CanvasWebViewPrefabView" || strObjectName == "Transparent" ||
                    strObjectName == "UserT_One" || strObjectName == "UserT_Two")
                {
                    ...
                    if (m_curvedVerts.Count == m_tesselatedVerts.Count)
                    {
                        ...
                        {
                            Vector3 prevVert = m_curvedVerts[i].position;

                            m_curvedVerts[i] = ICurveVertex(m_tesselatedVerts[i], mySettings.GetCyllinderRadiusInCanvasSpace(),
                                (myCanvas.transform as RectTransform).rect.size);


                            if (PerspectARConfig.bEnableInter)
                            {
                                //Interploation both the virtual screen and Transparent Window excluding both game indicators objects
                                if (strObjectName != "UserT_One" && strObjectName != "UserT_Two")
                                {
                                    Vector3 temp = Vector3.Lerp(prevVert, m_curvedVerts[i].position, Time.deltaTime * PerspectARConfig.fInter);
                                    UIVertex currentVertex = m_curvedVerts[i];
                                    currentVertex.position = new Vector3(temp.x, temp.y, temp.z);
                                    m_curvedVerts[i] = currentVertex;
                                }
                            }
                    ...
                    else
                    {
                        ...
                        for (int i = 0; i < m_tesselatedVerts.Count; i++)
                        {
                            ...
                            m_curvedVerts.Add(ICurveVertex(m_tesselatedVerts[i], mySettings.GetCyllinderRadiusInCanvasSpace(),
                                 (myCanvas.transform as RectTransform).rect.size));
                        }
                ...
                List<UIVertex> closestVerts = new List<UIVertex>();
                ...
                if (strObjectName == "Transparent")
                {
                    PerspectARConfig.fTransparentWindowDepth = transform.TransformPoint(GetMaximumXVertex(m_curvedVerts).position);
                }
        ...
        public CurvedUISettings FindParentSettings(bool forceNew = false)
        {
            ...
            {
                //Debug.Log("LeftA:" + mySettings.lAngle);
                //Debug.Log("RightB:" + mySettings.rAngle);
        ...
        //virtual screen's perspective compensation relative to the the user position and the large display
        UIVertex ICurveVertex(UIVertex input, float radius, Vector2 canvasSize)
        {

            Vector3 pos = input.position;//virtual screen vertices
            ...
            if (myCanvas.worldCamera != null)
            {
                ...
                float temp1 = (pos.x) - (userPose.x);
                theta = (temp1 / (canvasSize.x)) * mySettings.Angle * Mathf.Deg2Rad;
                margine = userPose.x;
            }
            else
            {
                theta = (pos.x / (canvasSize.x)) * mySettings.Angle * Mathf.Deg2Rad;
            }

            radius += pos.z; // change the radius depending on how far the element is moved in z direction from canvas plane

            if (this.transform.gameObject.name == "CanvasWebViewPrefabView")
            {
                m_curvedVertsAngles.Add(theta * Mathf.Rad2Deg);
            } 

            //In Unity scene, we are towards (-z)
            pos.x = Mathf.Sin(theta) * radius + margine;
        ...
        void TesselateGeometry(List<UIVertex> verts)
        {
            ...
            Vector2 ret = this.transform.GetComponent<RectTransform>().rect.size
                * this.transform.GetComponent<RectTransform>().localScale;


            Vector2 tessellatedSize = mySettings.GetTesslationSize(ret);
            ...
            {
#endif
                //Debug.Log(this.gameObject.name + ":" + verts.Count);
